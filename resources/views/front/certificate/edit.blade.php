                ...
                <form action="{{ route('front.certificate.update', $certificate->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="certificate_code" id="certificate_code"
                            class="form-input mt-1 block w-full" value="{{ $certificate->certificate_code }}" required>
                    </div>
                    <div class="mb-4">
                        ...
                        <select name="course_id" id="course_id" class="form-select mt-1 block w-full" required>
                            @foreach ($courses as $course)
                                <option value="{{ $course->id }}"
                                    {{ $course->id == $certificate->course_id ? 'selected' : '' }}>{{ $course->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="user_id" id="user_id" class="form-select mt-1 block w-full" required>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ $user->id == $certificate->user_id ? 'selected' : '' }}>{{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        ...
                        <input type="date" name="issued_date" id="issued_date" class="form-input mt-1 block w-full"
                            value="{{ date('Y-m-d', strtotime($certificate->issued_date)) }}" required>
