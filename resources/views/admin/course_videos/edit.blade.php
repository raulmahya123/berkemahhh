                ...
                <div class="item-card flex flex-row gap-y-10 justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        ...
                        <div class="flex flex-col">
                            <h3 class="text-blue-950 text-xl font-bold">{{ $video->course->name }}</h3>
                            <p class="text-slate-500 text-sm">{{ $video->course->category->name }}</p>
                        </div>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Teacher</p>
                        <h3 class="text-blue-950 text-xl font-bold">{{ $video->course->teacher->user->name }}</h3>
                    </div>
                </div>

                <hr class="my-5">

                <form method="POST" action="{{ route('admin.course_videos.update', $video->id) }}"
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name"
                            :value="old('name', $video->name)" required autofocus autocomplete="name" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="path_video" class="block mt-1 w-full" type="text" name="path_video"
                            :value="old('path_video', $video->path_video)" required autofocus autocomplete="path_video" />
